...
type MeatCount struct {
	Meat map[string]int `json:"meat"` // Use unordered map
}
...
func TestFire(t *testing.T) {
	...
	for meat, count := range expected {
		if actualCount, ok := actual[meat]; !ok || actualCount != count {
			t.Errorf("Expected count for %s: %d, got: %d", meat, count, actualCount)
		}
	}
